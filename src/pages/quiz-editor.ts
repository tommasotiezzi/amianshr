...
import type {
  Quiz, QuizQuestion, QuizType, QuestionType, QuestionConfig, AnswerKey, RankingItem,
...
export const createQuizEditorPage: PageFactory = (ctx) => {
  ...
  let quiz: Partial<Quiz> = isEdit ? {} : { title: '', description: '', quiz_type: 'logic', duration_minutes: 25 };
  ...
  if (isEdit) {
    q.fetchQuizWithQuestions(quizId, { signal: ctx.signal })
      .then((res) => {
  ...
  function renderFull() {
    ctx.container.innerHTML = `
      <div class="p-8 max-w-4xl mx-auto">
        <a href="#/quizzes" class="text-xs text-amia-400 hover:text-amia-600 transition-colors mb-3 inline-block">← Torna ai quiz</a>
        <h1 class="text-2xl font-semibold text-amia-950 tracking-tight mb-8">${isEdit ? 'Modifica quiz' : 'Nuovo quiz'}</h1>
        ...
        <div class="bg-white rounded-2xl border border-amia-100 p-6 mb-6">
          <div class="space-y-4">
            <div>
              ...
              <input type="text" id="quiz-title" value="${escapeAttr(quiz.title ?? '')}"
                placeholder="es. Quiz Logica"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <select id="quiz-type" class="w-full px-4 py-3 rounded-xl border border-amia-200 text-sm text-amia-900 bg-white">
                  ${QUIZ_TYPES.map((t) => `<option value="${t.value}" ${quiz.quiz_type === t.value ? 'selected' : ''}>${t.label}</option>`).join('')}
                </select>
              </div>
              <div>
                <label class="block text-xs font-medium text-amia-600 mb-1.5">Durata (minuti)</label>
                <input type="number" id="quiz-duration" value="${quiz.duration_minutes ?? ''}" placeholder="25"
                  class="w-full px-4 py-3 rounded-xl border border-amia-200 text-sm text-amia-900 placeholder:text-amia-300" />
              </div>
            </div>
            <div class="flex items-center gap-3 pt-2">
              <button id="save-quiz-btn" class="bg-amia-950 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-amia-900 active:scale-[0.98] transition-all">
                ${isEdit ? 'Salva quiz' : 'Crea quiz'}
              </button>
              ${isEdit ? `<button id="delete-quiz-btn" class="px-4 py-2.5 rounded-xl text-sm font-medium text-red-600 hover:bg-red-50 transition-colors">Elimina quiz</button>` : ''}
            </div>
          </div>
        </div>
  ...
  function renderQuestionsSection(): string {
    return `
      ...
      <button id="add-question-btn" class="mt-4 w-full py-4 rounded-2xl border-2 border-dashed border-amia-200 text-sm font-medium text-amia-400 hover:border-amia-300 hover:text-amia-600 transition-colors flex items-center justify-center gap-2">
  ...
  function bindQuestionListEvents() {
    ...
    ctx.$$<HTMLButtonElement>('.delete-question-btn').forEach((btn) => {
      ctx.on(btn, 'click', (e) => {
        ...
        supabase.from('quiz_questions').delete().eq('id', id).then(({ error }) => {
          if (ctx.signal.aborted) return;
  ...
  function openModal(existing: QuizQuestion | null) {
    openQuestionModal({
      quizId,
      ...
      if (ctx.signal.aborted || !result) return;
      if (existing) {
  ...
  function handleQuizSave() {
    ...
    if (!title) { showToast('Inserisci un titolo', 'error'); isSaving = false; return; }
    ...
    if (isEdit) {
      supabase.from('quizzes').update(payload).eq('id', quizId).then(({ error }) => {
        if (ctx.signal.aborted) return;
      ...
      supabase.from('quizzes').insert(payload).select().single().then(({ data, error }) => {
        if (ctx.signal.aborted) return;
        if (error) { showToast(`Errore: ${error.message}`, 'error'); isSaving = false; return; }
  ...
  function handleQuizDelete() {
    ...
    supabase.from('quizzes').delete().eq('id', quizId).then(({ error }) => {
      if (ctx.signal.aborted) return;
      if (error) { showToast(`Errore: ${error.message}`, 'error'); return; }
...
function loadingShell(): string {
  return `<div class="p-8 max-w-4xl mx-auto"><div class="flex justify-center py-20"><div class="spinner"></div></div></div>`;
}
    ...
    <div class="bg-white rounded-2xl border border-amia-100 p-5 question-card" data-id="${qn.id}">
      <div class="flex items-start justify-between mb-3">
        <div class="flex items-center gap-2 flex-wrap">
          ...
          ${qn.axis ? `<span class="text-[10px] font-medium text-accent bg-accent-light px-2 py-0.5 rounded">${AXIS_LABELS[qn.axis]}</span>` : ''}
        </div>
        <div class="flex items-center gap-1">
          <button class="edit-question-btn p-1.5 rounded-lg text-amia-400 hover:text-amia-600 hover:bg-amia-50 transition-colors" data-id="${qn.id}" title="Modifica">${iconEditSmall}</button>
          <button class="delete-question-btn p-1.5 rounded-lg text-amia-400 hover:text-red-600 hover:bg-red-50 transition-colors" data-id="${qn.id}" title="Elimina">${iconTrashSmall}</button>
        </div>
      </div>
      ...
      ${config?.image_url ? `
        <div class="mb-3 rounded-xl overflow-hidden border border-amia-100 bg-amia-50">
          <img src="${escapeAttr(config.image_url)}" alt="" class="w-full max-h-56 object-contain bg-white" loading="lazy" />
        </div>
      ` : ''}
        ...
        ${options.map((opt, i) => {
          ...
          return `
            <div class="flex items-center gap-2 text-xs ${isCorrect ? 'text-emerald-700 font-medium' : 'text-amia-500'}">
              <span class="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${isCorrect ? 'border-emerald-500 bg-emerald-50' : 'border-amia-200'}">
      ...
      <div class="space-y-1 mt-3">
        ${ordered.map((item, i) => `
          <div class="flex items-center gap-2 text-xs text-amia-600">
            <span class="w-5 h-5 rounded-full border-2 border-accent bg-accent-light flex items-center justify-center text-[10px] font-semibold text-accent shrink-0">${i + 1}</span>
            <span>${escapeText(item.label)}</span>
            ${item.axis_value != null ? `<span class="text-[10px] text-amia-300 ml-auto">val: ${item.axis_value}</span>` : ''}
          </div>
        `).join('')}
      </div>
    `;
...
const iconEditSmall = `<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>`;
const iconTrashSmall = `<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>`;
